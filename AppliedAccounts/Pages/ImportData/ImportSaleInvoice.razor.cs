...
namespace AppliedAccounts.Pages.ImportData
{
    public partial class ImportSaleInvoice
    {
        ...
        private async Task UpdateClient(DataRow _Row, long _RowID)              // Step 2.2.1
        {
            try
            {
        ...
        // Display Invoice Modal on click of data table view button
    ...
    public class ImportSaleInvoiceModel
    {
        ...
        {
            return SaleDetailsList.Where(a => (long)a["ID"] == _ID).ToList();
        }

        public void GetSelectedSale(long TranID)
        {
            var _master = SaleInvoiceList.Where(Row => (long)Row["ID"] == TranID).First();
            var _detail = SaleDetailsList.Where(Row => (long)Row["TranID"] == TranID).ToList();
            ...
            if (_detail.Count > 0)
            {
                ...
                {
                    decimal _TaxRate = AppliedDB.Functions.GetTaxRate(DBFile, (long)Item["Tax"]);
                    ...
                    _SaleInvoice.Inventory = AppliedDB.Functions.GetTitle(DBFile, Tables.Inventory, (long)Item["Inventory"]);
